...
class OsvcThread(threading.Thread, Crypt):
    ...
    def get_service_peers(self, path):
        """
        return sort list of service node peers pickup from all nodes service config
        """
        peers = set()
        for node in self.list_nodes():
            node_peers = self.daemon_status_data.get(["monitor", "nodes", node, "services", "config", path, "scope"],
                                                     default=[])
            peers.update(node_peers)
