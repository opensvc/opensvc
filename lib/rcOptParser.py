...
FORMATTER = optparse.TitledHelpFormatter(INDENT, INDENT+2)
FORMATTER.format_heading = lambda x: "\n"
...
def get_valid_actions(section, action):
    """
    Given a section and an action prefix, return the list of
    valid actions
    """
    valid_actions = []
    for candidate_action in sorted(ACTIONS[section]):
...
def format_options(section, action):
    """
    Format the possible options for a spectific action.
    """
    desc = ""
    parser = optparse.OptionParser(formatter=FORMATTER, add_help_option=False)
    for option in ACTIONS[section][action].get("options", []):
        option(parser)
    desc += SUBSEQUENT_INDENT + parser.format_option_help()
    return desc

def format_action(section, action, options=True):
    """
    ...
    desc += '\n\n'
    wrapper = TextWrapper(subsequent_indent=SUBSEQUENT_INDENT, width=78)
    text = SUBSEQUENT_INDENT + ACTIONS[section][action]["msg"]
    desc += wrapper.fill(text)
    desc += '\n'

    if options:
        desc += format_options(section, action)

    desc += '\n\n'
    return desc

def format_desc(svc=False, action=None, options=True):
    """
    Format and return a svcmgr parser help message, contextualized to display
    only actions matching the action argument.
    """
    desc = ""
    for section in sorted(ACTIONS):
        valid_actions = get_valid_actions(section, action)
        ...
        for valid_action in valid_actions:
            if svc and not hasattr(svc, valid_action):
                continue
            desc += format_action(section, valid_action, options=options)
    return desc[0:-2]
...
def get_action_from_args(parser, args, options):
    """
...
def get_parser(full=True):
    """
    Return an optparse parser with all possible options.
    """
    try:
    ...
    parser = optparse.OptionParser(
        version=__ver,
        usage=USAGE + format_desc(),
        add_help_option=False,
    )

    for option in OPT.values():
        if svclink() and option in SVCMGR_OPTS:
            continue
        option(parser)
    options, args = parser.parse_args()
    action = get_action_from_args(parser, args, options)
    ...
    options, args = parser.parse_args()
    parser.set_usage(USAGE + format_desc(action=action,
                                         options=options.parm_help))

    if options.parm_help or action not in supported_actions():
        print_context_help(parser, action)
    ...
    parser = configure_parser(parser, args)

    return parser

def configure_parser(parser, args):
    """
    Return the optparse parser, configured with contextualized options.
    """
    action = '_'.join(args)
    ...
    for option in GLOBAL_OPTS:
        option(parser)
    if svclink():
        for option in SVCMGR_OPTS:
            option(parser)
    for section in ACTIONS:
        if action not in ACTIONS[section]:
            continue
        for option in ACTIONS[section][action]["options"]:
            option(parser)
...
def print_short_help(parser):
    ...
    usage = USAGE + \
            format_desc(action=highlight_actions, options=False) + \
            "\n\nOptions:\n" + \
            "  -h, --help       Display more actions and options\n"
    parser.set_usage(usage)
    parser.error("Missing action")

def print_context_help(parser, action):
    """
    Trigger a parser error, which displays the help message contextualized
    for the action prefix.
    """
    parser.error("Invalid service action: %s" % str(action))
