...
class SyncDocker(resSync.Sync):
    ...
    def get_remote_images(self, node):
        ...
        cmd = rcEnv.rsh.split()+['-l', ruser, node, '--', rcEnv.paths.svcmgr, '-s', self.svc.svcname, "docker", "images", "-a", "--no-trunc"]
    ...
    def save_load(self, node, image):
        ruser = self.svc.node.get_ruser(node)
        save_cmd = [rcEnv.paths.svcmgr, "-s", self.svc.svcname, "docker", "save", self.image_id_name[image]]
