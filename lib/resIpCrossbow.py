...
class Ip(Res.Ip):
    ...
    def stopip_cmd(self):
        ...
        if self.gateway is not None:
            cmd=['route', '-q', 'delete', 'default', self.gateway, '>/dev/null', '2>&1']
            r, o, e = self.vcall(cmd)
            ret += r
            out += o
            err += e
        objext = self.rid.replace('#', '')
        cmd=['ipadm', 'delete-addr', self.stacked_dev+'/v4osvcres'+objext ]
        r, o, e =  self.vcall(cmd)
        ret += r
        out += o
        err += e
        tst = Popen(['ipadm', 'show-addr', self.stacked_dev+'/' ],
    ...
    def startip_cmd(self):
        ret, out, err = (0, '', '')
        tst = Popen(['ipadm', 'show-if', self.stacked_dev, '2>/dev/null' ],
        ...
        n = 5
        while n != 0:
            cmd=['ipadm', 'create-addr', '-t', '-T', 'static', '-a', self.addr+self._dotted2cidr(), self.stacked_dev+'/v4osvcres'+objext ]
            r, o, e = self.vcall(cmd)
            if r == 0:
                break
        ...
        err += e
        if self.gateway is not None:
            cmd=['route', '-q', 'add', 'default', self.gateway, '>/dev/null', '2>&1' ]
            r, o, e = self.vcall(cmd)
            ret += r
            out += o
            err += e
