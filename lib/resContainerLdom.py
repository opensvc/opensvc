...
class Ldom(Res.Resource):
    startup_timeout = 600
    shutdown_timeout = 60

    def __init__(self, name, optional=False, disabled=False):
        Res.Resource.__init__(self, rid="ldom", type="container.ldom",
    ...
    def start(self):
        if self.is_up():
            self.log.info("ldom container %s already started" % self.name)
            return
        self.ldom_start()
    ...
    def stop(self):
        if not self.is_up():
            self.log.info("ldom container %s already stopped" % self.name)
            return
        self.ldom_stop()
