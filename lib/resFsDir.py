...
class FsDir(Res.Resource):
    ...
    def create(self):
        ...
        if not self.check_perms():
            self.log.info("set %s perms to %s" % (self.path, str(self.perms)))
            os.chmod(self.path, int(str(self.perms), 8))
    ...
    def check_perms(self):
        ...
        if not os.path.exists(self.path):
            return True
        perms = oct(stat.S_IMODE(os.stat(self.path).st_mode))
        perms = str(perms).lstrip("0")
        if perms != str(self.perms):
            self.status_log('perms should be %s but is %s'%(str(self.perms), perms))
            return False
        return True
    ...
    def __str__(self):
        return "%s path=%s user=%s group=%s perms=%s" % (Res.Resource.__str__(self),\
                self.path, str(self.user), str(self.group), str(self.perms))
